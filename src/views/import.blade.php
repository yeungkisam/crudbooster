...
@section('content')
    ...
    @if (
        ($button_show_data ?? false) ||
            ($button_reload_data ?? false) ||
            ($button_new_data ?? false) ||
            ($button_delete_data ?? false) ||
            ($index_button ?? false) ||
            ($columns ?? false))
    ...
    @if (Request::get('file') && Request::get('import'))
        <ul class='nav nav-tabs'>
            ...
            <li style="background:#eeeeee"><a style="color:#111" href='#'><i class='fa fa-cogs'></i> Adjustment
                    &raquo;</a></li>
                        ...
                        $(function() {
                            ...
                            var int_prog = setInterval(function() {
                                ...
                                $.post("{{ CRUDBooster::mainpath('do-import-chunk?file=' . Request::get('file')) }}", {
                                    ...
                                    $('#status-import').html(
                                        "<i class='fa fa-spin fa-spinner'></i> Please wait importing... (" +
                                        resp.progress + "%)");
                                    $('#progress-import').attr('aria-valuenow', resp.progress);
                                    if (resp.progress >= 100) {
                                        $('#status-import').addClass('text-success').html(
                                            "<i class='fa fa-check-square-o'></i> Import Data Completed !");
            ...
            <div class="box-footer" id='upload-footer' style="display:none">
                <div class='pull-right'>
                    <a href='{{ CRUDBooster::mainpath('import-data') }}' class='btn btn-default'><i
                            class='fa fa-upload'></i> Upload Other File</a>
    ...
    @if (Request::get('file') && !Request::get('import'))
        <ul class='nav nav-tabs'>
            ...
            <li style="background:#eeeeee"><a style="color:#111" href='#'><i class='fa fa-cloud-download'></i>
                    Importing &raquo;</a></li>
        </ul>
            ...
            <form method='post' id="form" enctype="multipart/form-data" action='{{ $action }}'>
                ...
                <div class="box-body table-responsive no-padding">
                    <div class='callout callout-info'>
                        ...
                        * Warning !, Unfortunately at this time, the system can't import column that contains image or photo
                        url.
                    </div>
                    @push('head')
                        <style type="text/css">
            ...
            <form method='post' id="form" enctype="multipart/form-data" action='{{ $action }}'>
                ...
                <div class="box-body">
                    ...
                    <div class='callout callout-success'>
                        ...
                        * If you have a big file data, we can't guarantee. So, please split those files into some parts of
                        file (at least max 5 MB).<br />
                        * This tool is generate data automatically so, be carefull about your table xls structure. Please
                        make sure correctly the table
                        structure.<br />
                        * Table structure : Line 1 is heading column , and next is the data. (For example, you can export
                        any module you wish to XLS format)
                    </div>
