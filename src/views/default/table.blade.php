            ...
            <tr class="active">
                ...
                @if ($button_table_action)
                    @if (CRUDBooster::isUpdate() || CRUDBooster::isDelete() || CRUDBooster::isRead())
                        <th width='{{ isset($button_action_width) ? $button_action_width : 'auto' }}'
                            style="text-align:right">{{ cbLang('action_label') }}</th>
                    @endif
                @endif
            </tr>
        </thead>
            ...
            $(function() {
                ...
                $(".toggle_advanced_report").click(function() {
                    ...
                    if (toggle_advanced_report_boolean == 1) {
                        ...
                        $(this).html(
                            "<i class='fa fa-minus-square-o'></i> {{ cbLang('export_dialog_show_advanced') }}"
                            );
                ...
                $(".filter-combo").change(function() {
                    ...
                    switch (n) {
                        ...
                        case 'like':
                        case 'not like':
                            filter_value.attr('placeholder',
                                '{{ cbLang('filter_eg') }} : {{ cbLang('filter_lorem_ipsum') }}').prop(
                                'disabled', false);
                            break;
                        case 'asc':
                            filter_value.prop('disabled', true).attr('placeholder',
                                '{{ cbLang('filter_sort_ascending') }}');
                            break;
                        case 'desc':
                            filter_value.prop('disabled', true).attr('placeholder',
                                '{{ cbLang('filter_sort_descending') }}');
                            break;
                        case '=':
                            filter_value.prop('disabled', false).attr('placeholder',
                                '{{ cbLang('filter_eg') }} : {{ cbLang('filter_lorem_ipsum') }}');
                            break;
                        case '>=':
                            filter_value.prop('disabled', false).attr('placeholder',
                                '{{ cbLang('filter_eg') }} : 1000');
                            break;
                        case '<=':
                            filter_value.prop('disabled', false).attr('placeholder',
                                '{{ cbLang('filter_eg') }} : 1000');
                        ...
                        case 'in':
                            filter_value.prop('disabled', false).attr('placeholder',
                                '{{ cbLang('filter_eg') }} : {{ cbLang('filter_lorem_ipsum_dolor_sit') }}'
                                );
                            break;
                        case 'not in':
                            filter_value.prop('disabled', false).attr('placeholder',
                ...
                <div class="modal-content">
                    ...
                    <form method='get' action=''>
                        <div class="modal-body">
                            ...
                            <?php if (isset($col['image']) || isset($col['download']) || ($col['visible'] ?? 0) === false) {
                                continue;
                            } ?>
                        ...
                        <div class="modal-footer" align="right">
                            <button class="btn btn-default" type="button"
                                data-dismiss="modal">{{ cbLang('button_close') }}</button>
                            ...
                            <button class="btn btn-primary btn-submit"
                                type="submit">{{ cbLang('button_submit') }}</button>
                        </div>
                        {!! CRUDBooster::getUrlParameters(['filter_column', 'lasturl']) !!}
                        <input type="hidden" name="lasturl"
                            value="{{ Request::get('lasturl') ? Request::get('lasturl') : Request::fullUrl() }}">
                    </form>
                </div>
                <!-- /.modal-content -->
            </div>
                        ...
                        <div class="modal-body">
                            <div class="form-group">
                                ...
                                <input type='text' name='filename' class='form-control' required
                                    value='Report {{ $module_name ?? false }} - {{ date('d M Y') }}' />
                            ...
                            <div class="form-group">
                                ...
                                <input type='number' name='limit' class='form-control' required value='100'
                                    max="100000" min="1" />
                            ...
                            <p><a href='javascript:void(0)' class='toggle_advanced_report'><i
                                        class='fa fa-plus-square-o'></i> {{ cbLang('export_dialog_show_advanced') }}</a>
                            </p>
                            ...
                            <div id='advanced_export' style='display: none'>
                                ...
                                <div class="form-group">
                                    ...
                                    <select class='form-control' name='page_size'>
                                        <option
                                            <?= optional($setting ?? false)->default_paper_size == 'Letter' ? 'selected' : '' ?>
                                            value='Letter'>Letter</option>
                                        <option
                                            <?= optional($setting ?? false)->default_paper_size == 'Legal' ? 'selected' : '' ?>
                                            value='Legal'>Legal</option>
                                        <option
                                            <?= optional($setting ?? false)->default_paper_size == 'Ledger' ? 'selected' : '' ?>
